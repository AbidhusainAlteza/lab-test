            ...
            <div class="col-md-12">
                <div class="ms-panel ">
                    ...
                    <div class="ms-panel-body">
                        <form class=" assign_area_add_form" action="{{url('admin/assign-area-edit-post')}}" method="POST" enctype="multipart/form-data" novalidate>
                            @csrf
                            <input type="hidden" name="assign_area_id" value="{{$assign_areas->id}}">
                                    ...
                                    <div class="input-group">
                                        <select name="pincode_id" id="pincode_id" class="form-control" required>
                                            ...
                                            @foreach ($manag_area_pincodes as $manag_area_pincode)
                                                <option value="{{$manag_area_pincode->id}}" {{$manag_area_pincode->id == $assign_areas->admin_manag_area_id ? "selected":""}}>{{$manag_area_pincode->pincode}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4 mb-3">
                                    ...
                                    <div class="input-group">
                                        <select name="vendor_id" id="vendor_id" class="form-control" required="required">
                                            ...
                                            <option value="{{$vendor->id}}" {{$vendor->id == $assign_areas->vendor_id ? "selected":""}}>{{$vendor->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
